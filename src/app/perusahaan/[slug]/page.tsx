...
const COMPANY_DATABASE: Record<string, CompanyData> = {
  ...
  'bank-mandiri': {
    ...
    csrEmail: 'csr@bankmandiri.co.id',
    description: 'PT Bank Mandiri (Persero) Tbk menyalurkan program TJSL berbasis keberlanjutan Mandiri Bersama Mandiri (MBM) untuk inklusi keuangan dan kemandirian ekonomi.',
    tjslFocus: 'Inklusi Keuangan, Wirausaha Muda, & Layanan Kesehatan Masyarakat.',
    csrPillars: [
      { title: 'Wirausaha Muda Mandiri', desc: 'Inkubasi bisnis dan pendanaan awal startup mahasiswa.', asnaf: 'Fisabilillah' },
      { title: 'Mandiri Kesehatan', desc: 'Bantuan fasilitas posyandu dan pencegahan stunting desa.', asnaf: 'Fakir / Miskin' },
    ],
    activePrograms: [
      { title: 'Mandiri Inklusi Keuangan Desa', budget: 'Rp 1.5 Miliar', location: 'Jawa Tengah & Jawa Timur' },
    ],
    lastUpdated: '15 September 2026',
  ...
  pln: {
    ...
    province: 'DKI Jakarta',
    address: 'Jl. Trunojoyo Blok M-I No. 135, Kebayoran Baru, Jakarta Selatan',
    website: 'https://www.pln.co.id',
    csrEmail: 'tjsl@pln.co.id',
    description: 'PT PLN (Persero) menyalurkan TJSL PLN Peduli untuk peningkatan rasio elektrifikasi desa, konversi energi hijau, serta bantuan usaha mikro.',
    tjslFocus: 'PLN Peduli Elektrifikasi, Pelestarian Lingkungan, & Usaha Mikro Listrik.',
    csrPillars: [
      { title: 'PLN Peduli Lingkungan', desc: 'Penanaman pohon penyerap karbon dan konservasi muara.', asnaf: 'Maslahat Umum' },
    ],
    activePrograms: [
      { title: 'Listrik Gratis Desa Terpencil', budget: 'Rp 5.0 Miliar', location: 'Papua & Nusa Tenggara' },
    ],
    lastUpdated: '15 September 2026',
    sources: ['Laporan Tahunan PLN Peduli 2025'],
  },
  'adaro-energy': {
    slug: 'adaro-energy',
    ...
    address: 'Menara Karya Lt. 23, Jl. H.R. Rasuna Said Blok X-5 Kav. 1-2, Jakarta Selatan',
    website: 'https://www.adaro.com',
    csrEmail: 'csr@adaro.com',
    description: 'PT Adaro Energy Indonesia Tbk menjalankan program CSR Adaro Nyalakan Ilmu dan Adaro Sehat untuk masyarakat sekitar tambang.',
    tjslFocus: 'Pendidikan Vokasi Tambang, Penyediaan Air Bersih, & Kesehatan Ibu-Anak.',
    csrPillars: [
      { title: 'Adaro Nyalakan Ilmu', desc: 'Beasiswa SMK Vokasi dan pelatihan ketrampilan mekanik.', asnaf: 'Fisabilillah' },
    ],
    activePrograms: [
      { title: 'Adaro Air Bersih Desa Tambang', budget: 'Rp 2.0 Miliar', location: 'Kalimantan Selatan' },
    ],
    lastUpdated: '15 September 2026',
    sources: ['Sustainability Report Adaro 2025'],
  },
  'astra-international': {
    ...
    csrEmail: 'csr@astra.co.id',
    description: 'PT Astra International Tbk menyalurkan program CSR terpadu melalui Kampung Berseri Astra (KBA) dan Desa Sejahtera Astra (DSA).',
    tjslFocus: 'Pendidikan, Kesehatan, Lingkungan, & Kewirausahaan Mandiri.',
    csrPillars: [
      { title: 'Kampung Berseri Astra', desc: 'Pengembangan desa mandiri terintegrasi 4 pilar.', asnaf: 'Maslahat Umum' },
    ],
    activePrograms: [
      { title: 'Desa Sejahtera Astra (DSA) 2026', budget: 'Rp 6.0 Miliar', location: '34 Provinsi Indonesia' },
    ],
    lastUpdated: '15 September 2026',
    sources: ['Laporan Keberlanjutan Astra 2025'],
  },
};

export function generateStaticParams() {
  return Object.keys(COMPANY_DATABASE).map((slug) => ({ slug }));
}

export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const company = COMPANY_DATABASE[params.slug];
  if (!company) {
    return { title: 'Perusahaan Tidak Ditemukan | CSRmatics' };
  }

  const title = `Program CSR & TJSL ${company.name} | Informasi & Peluang Kemitraan`;
  const description = `Informasi alokasi dana CSR, pilar TJSL, program sosial aktif, dan tata cara pengajuan proposal kemitraan CSR ke ${company.name}.`;
  ...
  return {
    ...
    keywords: [
      ...
      `TJSL ${company.name}`,
      `Alokasi CSR ${company.ticker}`,
      `Proposal CSR ${company.name}`,
      `Dana CSR ${company.name}`,
    ],
    alternates: {
      canonical: `https://csrmatics.com/perusahaan/${company.slug}`,
    },
    openGraph: {
      title,
      description,
      url: `https://csrmatics.com/perusahaan/${company.slug}`,
      siteName: 'CSRmatics',
      locale: 'id_ID',
      type: 'article',
      images: [
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Perusahaan', item: 'https://csrmatics.com/database-perusahaan' },
          { '@type': 'ListItem', position: 3, name: company.name, item: `https://csrmatics.com/perusahaan/${company.slug}` },
        ],
      },
      {
        '@type': 'Organization',
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans relative overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-slate-950/80 border-b border-slate-800/60">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-6 text-sm font-medium text-slate-300">
            ...
            <Link href="/database-perusahaan" className="text-emerald-400 font-bold">Directory</Link>
      ...
      <main className="py-12 px-6 max-w-5xl mx-auto space-y-10">
        ...
        <nav className="flex items-center gap-2 text-xs text-slate-400">
          ...
          <Link href="/database-perusahaan" className="hover:text-white">Directory Perusahaan</Link>
          <ChevronRight className="w-3.5 h-3.5" />
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <span className="px-3.5 py-1.5 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-semibold border border-emerald-500/30 flex items-center gap-1.5">
                ...
                <span>Verified HTTP 200 OK</span>
              </span>
            </div>
          </div>
        ...
        <section className="p-6 rounded-2xl bg-indigo-950/40 border border-indigo-800/60 space-y-4">
          <h3 className="font-bold text-white text-base">Cara Mengajukan Proposal CSR ke {company.name}</h3>
          <p className="text-xs text-slate-300 leading-relaxed">
            Lembaga sosial / NGO terverifikasi dapat mengajukan proposal terstruktur langsung ke tim TJSL {company.name} melalui matching engine AI CSRmatics tanpa perlu cold outreach manual.
          </p>
          <div className="pt-2">
            <Link
              href="/pricing"
            ...
            >
              <span>Ajukan Proposal via CSRmatics Engine</span>
